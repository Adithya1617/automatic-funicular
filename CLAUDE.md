...
| Service event | (new in slice H5, ad-hoc flow added H10, split by `kind` in H11) replaces `orders` | Linked to one bike. `kind` (`service` \| `repair` \| `wash`, default `service`) discriminates the three section pages. `service_template_id` / `..._version_id` are **nullable** — the primary UX is ad-hoc (`ServiceService.createAdHoc(bikeId, kind, lines[])` creates + completes in one tx). `service` = Engine oil only; `repair` = any parts; `wash` = zero lines, no stock movements. Template-driven events still capture the active version at create time (Path A snapshot). Status workflow: `requested → in_progress (UI label "Under service") → completed`, `cancelled` terminal. Stock deducts **only** on reaching `completed` (via `ServiceService.setStatus` or a one-shot ad-hoc create that targets `completed`); `requested`/`in_progress` record planned lines without touching stock. The Services UI shows oil-only lines/cost for `kind='service'`. |
| Stock take | `stock_takes` | Unchanged. |
| Dashboard | rebuilt slice H6, alerts in H11 | Bike-centric tiles: cost per bike, cost per bike type, parts consumed, top-consumed parts, low stock, reorder suggestions, service volume by bike type, services by bike, wastage, theoretical service cost per template, plus **Service due / Wash due** maintenance alert panels. |
...
| H10 | pending | **Ad-hoc "Start servicing" flow.** Migration 0012 nullable `service_events.service_template_id` / `..._version_id`; new `ServiceService.createAdHoc(bikeId, lines[])` creates + completes in one tx, deducting stock. (The original `QuickServiceDialog` was superseded by H11's three dialogs.) |
| H11 | done | **Split servicing into Service / Repair / Wash + maintenance alerts.** Migration 0013 adds `service_events.kind`. `createAdHoc` carries `kind` (wash = zero lines, no movements). Three pages (`/services` oil-only, `/repairs` any parts, `/wash`) each with their own dialog (`ServiceDialog`/`RepairDialog`/`WashDialog`). Constants `SERVICE_INTERVAL_DAYS=45`, `WASH_INTERVAL_DAYS=15`, `MAINTENANCE_ALERT_DAYS=3`, `DEFAULT_SERVICE_OIL_ML=800`. Dashboard: `maintenanceSchedule` + `serviceVolumeByBike` endpoints drive "Service due" / "Wash due" alert panels (countdown from last completed event of that kind, <3d highlighted, Start action) and a "Services by bike" tile; Bikes page gains service/wash countdown columns. |
